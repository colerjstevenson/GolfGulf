...
def make_quintile_legend(
    ...
    *,
    ...
    """
    Legend is placed outside the map (right side), with range labels.
    """
    handles: list[Patch] = []
...
def make_quintile_map(
    ...
    *,
    ...
    fig, ax = plt.subplots(figsize=(7.8, 7.6))  # slightly wider for right-side legend
    ax.set_aspect("equal")
    ax.axis("off")
    ax.set_title(title)
    ...
    # Overlays (buffers first, then course outlines)
    if show_buffers and len(golf) > 0:
        buf = golf.copy()
        buf["geometry"] = buf.geometry.buffer(buffer_m)
        buf.plot(ax=ax, facecolor="none", edgecolor="black", linewidth=0.6, alpha=0.25, zorder=3)

    if show_courses and len(golf) > 0:
        golf.plot(ax=ax, facecolor="none", edgecolor="black", linewidth=1.0, zorder=4)

    # Tract boundaries LAST (so they are visible and black)
    tracts.boundary.plot(ax=ax, linewidth=0.5, edgecolor="black", zorder=10)

    if annotate_courses and len(golf) > 0:
...
def main() -> None:
    ...
    var_specs = [
        ("median_hh_income_2020", "Vancouver census tracts: Median household income (quintiles)", "map_income_quintiles", "income"),
        ("pct_renters", "Vancouver census tracts: Renter households (%) (quintiles)", "map_renters_quintiles", "percent"),
        ("pct_65plus", "Vancouver census tracts: Population aged 65+ (%) (quintiles)", "map_seniors_quintiles", "percent"),
        ("pop_2021", "Vancouver census tracts: Population (2021) (quintiles)", "map_population_quintiles", "count"),
        ("pop_density_per_km2", "Vancouver census tracts: Population density (per km²) (quintiles)", "map_density_quintiles", "density"),
    ]
